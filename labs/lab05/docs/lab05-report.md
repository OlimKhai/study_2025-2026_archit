# Лабораторная работа: Менеджер паролей pass и управление конфигурацией

## Студент
- **ФИО:** Хайдаров Олим Фирдавсович
- **Студ. билет** 1032242468 
- **Группа:** НБИбд-02-25
- **Дата выполнения:** 14 марта 2025

---

## Цель работы
Изучение и настройка менеджера паролей `pass`, интеграция с браузером через `browserpass`, а также освоение утилиты `chezmoi` для управления конфигурационными файлами.
...
Результат: Хранилище инициализировано. Успешно

### 4. Настройка Git синхронизации

 Настройка идентичности Git:
git config --global user.email "1032242468@rudn.ru"
git config --global user.name "Olim Khaidarov"

  Инициализация Git репозитория:
pass git init

 Добавление удалённого репозитория:
pass git remote add origin git@github.com:OlimKhai/study2025-2026_archit.git

 Отправка изменений:
pass git push --set-upstream origin master

Результат: Репозиторий настроен и синхронизирован


### 5. Настройка SSH ключа для GitHub

 Генерация SSH ключа:
ssh-keygen -t ed25519 -C "1032242468@rudn.ru"
...
Результат: SSH ключ добавлен на GitHub


### 6. Установка и настройка browserpass

  Установка browserpass:
sudo dnf install browserpass

 Создание конфигурации для Firefox:
nano ~/.mozilla/native-messaging-hosts/browserpass.json

Содержимое browserpass.json:

{
    "name": "com.github.browserpass.native",
    "description": "Browserpass native host",
    "path": "/usr/bin/browserpass-native",
    "type": "stdio",
    "allowed_origins": [
...
Результат: Расширение установлено и настроено
...
Результат: Пароли успешно сохраняются и извлекаются
...
gh repo create dotfiles --template="yamadharama/dotfiles-template" --private
...
Результат: chezmoi настроен и работает соответствующе


### 9. Автоматическая фиксация изменений

[git]
autoCommit = true
autoPush = true

Результат: Автоматическая отправка включена
...
В ходе лабораторной работы были освоены:
    ...
    Генерация и использование GPG ключей для шифрования
    Синхронизация хранилища паролей с GitHub через Git
    Интеграция с браузером через browserpass
    Управление конфигурационными файлами с помощью chezmoi
    Настройка автоматической синхронизации изменений
